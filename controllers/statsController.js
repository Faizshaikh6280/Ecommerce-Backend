...
const { calcChangeRate, getNoOfProductBasedOnCategory } = require("../utils/helper");
...
exports.getDashBoardStats = catchAsync(async (req, res, next) => {
  ...
  if (mycache.has("dashboard-stats")) {
    ...
    const [
      ...
      orderModel.find({}).select(["_id","status","orderItems","total","discount"])
    ...
    const productCountBasedOnCategory = await getNoOfProductBasedOnCategory(productCount)
...
exports.getPieChart = catchAsync(async(req,res,next)=>{
  ...
  if (mycache.has("pieChart-data")) {
    ...
    const [productOutOfStock,productCount,allOrders] = await Promise.all([productModel.countDocuments({stock : 0}),productModel.countDocuments(),orderModel.find()]);
    ...
    //1. no. of orders based on status.
      ...
      const noOfProductsBasedOnCategory = await getNoOfProductBasedOnCategory(productCount)
      ...
      const grossAmount = allOrders.reduce((acc,order)=> acc + order.total || 0,0);
      const discountAmount = allOrders.reduce((acc,order)=> acc + order.discount || 0,0);

      const productionCost = allOrders.reduce((acc,order)=> acc + order.shippingCharges || 0,0);


      const burnt = allOrders.reduce((acc,order)=> acc + order.
      tax || 0,0);
      const marketingCost = Math.round(grossAmount * (30/100))
      const netMargin = grossAmount - discountAmount - productCount - burnt - marketingCost
      pieChart = {
